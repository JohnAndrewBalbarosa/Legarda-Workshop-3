...
export function createUserHostServer({
  host = process.env.USER_HOST || process.env.HOST || '127.0.0.1',
  port = Number.parseInt(process.env.USER_PORT || process.env.PORT || '5174', 10),
  presenterWs = process.env.PRESENTER_WS || 'ws://10.250.250.1:5050',
  defaultUserId = process.env.USER_ID || 'user-1',
  defaultSeat = process.env.USER_SEAT || 'A1',
} = {}) {
  const effectivePort = Number.isNaN(port) ? 5174 : port;

  const staticRoutes = new Map([
    ['/user', { filePath: 'user.html', contentType: 'text/html; charset=utf-8' }],
    ['/index.html', { filePath: 'user.html', contentType: 'text/html; charset=utf-8' }],
  ...
  const server = http.createServer((request, response) => {
    ...
    if (request.method === 'GET' && pathname === '/') {
      const launchUrl = new URL('/user', `http://${request.headers.host ?? `localhost:${effectivePort}`}`);
      launchUrl.searchParams.set('id', requestUrl.searchParams.get('id') || defaultUserId);
      launchUrl.searchParams.set('seat', requestUrl.searchParams.get('seat') || defaultSeat);
...
export default createUserHostServer;

const entryFilePath = process.argv[1];

if (entryFilePath && currentFilePath === entryFilePath) {
  const server = createUserHostServer();
  server
    .start()
    .then(({ host, port }) => {
      console.log(`User host server listening on http://${host}:${port}`);
    })
    .catch((error) => {
      console.error('Failed to start user host server.');
      console.error(error);
      process.exitCode = 1;
    });
}
